...
describe("generateMap", () => {
  it("is deterministic: same seed produces identical terrain and elevation arrays", () => {
    ...
    expect(Array.from(a.map.elevation)).toEqual(Array.from(b.map.elevation));
    expect(Array.from(a.map.elevationRaw)).toEqual(Array.from(b.map.elevationRaw));
    expect(Array.from(a.map.riverFlow)).toEqual(Array.from(b.map.riverFlow));
    expect(Array.from(a.map.riverNext)).toEqual(Array.from(b.map.riverNext));
  ...
  it("never produces NaN or out-of-range elevations", () => {
    const { map } = generateMap(
      createRng(777),
      options({ roughness: "mountainous", size: "large" }),
    );
    for (let i = 0; i < map.elevation.length; i++) {
      ...
      expect(Number.isFinite(map.elevationRaw[i] as number)).toBe(true);
    }
  });
